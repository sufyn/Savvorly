<?php
    ...
    if(isset($_POST['like']) && !empty($_POST['like'])){
		...
		if($for_user != $user_id) {
			...
			weet::create('notifications' , $data);
			
		} 


		User::create('likes', array('user_id' => $user_id, 'post_id' => $weet_id));

		echo `<div class="tmp d-none">
             `+ weet::countLikes($weet_id) +`            
		</div>` ;
	}
    if(isset($_POST['unlike']) && !empty($_POST['unlike'])){
    	$user_id  = $_SESSION['user_id'];
    	$weet_id = $_POST['unlike'];
    	$get_id   = $_POST['user_id'];
		$for_user = weet::getData($weet_id)->user_id;
